...
interface NonStandardComponent {
  id: number;
  komponen: string;
  satuan: string;
  bobot: number;
...
export default function VerifyKomponenNonStandarPage() {
  ...
  const [components, setComponents] = useState<NonStandardComponent[]>([]);
  ...
  // Fetch non-standard components data
  useEffect(() => {
    const fetchComponents = async () => {
      ...
      try {
        ...
        // Fetch non-standard components
        const response = await fetch('/api/usulan/bangunan-gedung/asb/komponen-nonstandar', {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        ...
        if (response.ok) {
          ...
          componentList.forEach((comp: NonStandardComponent) => {
