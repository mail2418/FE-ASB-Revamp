...
const BuildingContainer = dynamic(() => import('@/components/UsulanBangunan/3DComponents/BuildingContainer'), { ssr: false });

export default function RightPanel3D() {
  const [standardFormState, setStandardFormState] = useState<BuildingFormState>({});

  useEffect(() => {
    // Load standard components data for visualization from localStorage (if available)
